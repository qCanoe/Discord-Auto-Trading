...
11. 减仓/止盈/平仓识别规则（重要）：
    ...
    - "全部止盈" / "全平" / "清仓" / "全部逃命" / "全部跑路" / "全部撤退" → action="close"
    - 只说"可以止盈"或"止盈"但没有指定比例 → action="reduce", reduce_pct=null
    - "移动止损到保本" / "移动保本损" / "保本止损" → sl 字段填 0（表示移至保本，无具体价格）
    - "逃命" / "跑路" / "撤退" 后面跟着某个价格区间，表示"到达该价格时平仓"，应识别为 action="close" 或 action="reduce"
12. 消息中含有"止盈"、"减仓"、"平仓"、"出货"等操作动词，即使语气口语化也应视为交易指令，不能返回 {"action": null}
13. 消息中含有"撤掉之前的单子"、"更新这个"、"重新挂单"、"替换之前"等字样时，在 JSON 中额外加入 "cancel_previous": true 字段，表示执行前需先取消该 symbol 的所有未成交挂单
14. 【假设性语境识别（极重要）】
    - 消息中含有"如果成交" / "一旦成交" / "成交后" / "成本会变成" / "成本会自动变成" 等描述假设场景的短语时，说明消息在讨论某个已挂单子成交后的应对策略，而非发出新的开仓指令。
    - 此时消息中出现的价格（例如"如果71288成交"中的71288）是对已有挂单的引用，不应作为新的 entries 入场价。
    - 若消息整体是"如果XX成交，则在YY逃命/平仓"这类条件性策略说明，且没有明确的"现在开仓"指令，应返回 {"action": null}；
    - 若其中包含明确的平仓动作（如"全部逃命"、"全部跑路"），则视为 action="close"，不填写 entries，take_profits 中的价格来自"逃命/平仓"对应的目标价。
15. "成本" / "均价" / "成本价" 表示持仓的平均开仓成本，不是止盈/止损价格，也不是新的入场价格，不应填入 entries 或 take_profits。
"""
    ...
    async def parse(self, text: str) -> Optional[Signal]:
        ...
        try:
            ...
            signal = Signal(
                ...
                cancel_previous=bool(data.get("cancel_previous", False)),
                raw_text=text,
            )
